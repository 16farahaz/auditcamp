...
// Function to fetch and aggregate state data for a specific `idste`
const fetchAndAggregateStates = async (idste) => {
    ...
    const promises = collectionNames.map(async (collectionName) => {
        ...
        const aggregationPipeline = [
            {
                $match: { idste: idste } // Match documents with the specified idste
            },
            {
                $group: {
                    _id: {
...
const getChartData = async (req, res) => {
    const idste = req.params.idste; // Get the idste from request parameters
    try {
        const stateCounts = await fetchAndAggregateStates(idste);
